...
//	private float[] colors;
	...
	public SFSpecialPerlinNoise(int width,int height,float[] weights,float colors[]){
		...
		for(int i=0;i<height;i++){
			for(int j=0;j<width;j++){
				...
				for(int k=0;k<weights.length;k++){
					float color=getOctaveValue(u, v, oct)*weights[k];
					colorR+=colors[3*k+0]*color;
					colorG+=colors[3*k+1]*color;
					colorB+=colors[3*k+2]*color;
					if(k==0){
						oct=1;
					}else{
						oct*=2;
				...
				buffer.put((byte)colorR); 
				buffer.put((byte)colorG);
				buffer.put((byte)colorB);
			}
		}
		this.setWidth(width);
