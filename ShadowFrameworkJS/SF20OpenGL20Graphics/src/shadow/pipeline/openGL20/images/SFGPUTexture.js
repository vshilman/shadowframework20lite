...
function SFGPUTexture(){
}

SFGPUTexture.prototype = {
	...
	//Step 5: generate frame buffer. Bind to buffer		int nfbo[]=new int[1];//Warning: Not well Identified 
		...
		fbo  = nfbo[0];
	//Step 8: texture bindings (txo and txo4)		gl.glBindTexture(GL.GL_TEXTURE_2D, texture_object);//Warning: Not well Identified 
		gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, GL.GL_RGBA8, data.getWidth(), data.getHeight(), 0,	             GL.GL_RGB,GL.GL_UNSIGNED_BYTE, null);
	...
	//Step 7: New Viewport for texture generation				gl.glViewport(0, 0, data.getWidth(),data.getHeight());//Warning: Not well Identified 
		gl.glFramebufferTexture2D(GL.GL_FRAMEBUFFER,				GL.GL_COLOR_ATTACHMENT0,				GL2.GL_TEXTURE_2D,texture_object,0);
	//Step 1: generate a texture object to store the buffer.		int n[]=new int[1];//Warning: Not well Identified 
	...
	//Step 8: texture bindings (txo and txo4) 			gl.glActiveTexture(GL.GL_TEXTURE0);//Warning: Not well Identified 
		...
		gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, GL2.GL_DEPTH_COMPONENT,data.getWidth(), data.getHeight(), 0,	             GL2.GL_DEPTH_COMPONENT,GL.GL_FLOAT, null);
		gl.glFramebufferTexture2D(GL.GL_FRAMEBUFFER,				GL.GL_DEPTH_ATTACHMENT,				GL.GL_TEXTURE_2D,depthBuffer,0);
