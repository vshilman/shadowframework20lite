...
SFInputStreamBinary.prototype["readBinaryDataValue"]=function(bitSize){
			var byteSize = ((bitSize - 1) >> 3) + 1;
		
			var bytes = this.readBytes(byteSize);
			
			var value = 0;
			for (var j = 0; j < byteSize; j++) {
				var byteValue = bytes[ j];
				value += ((byteValue >= 0 ? byteValue : 256 + byteValue) << (8 * j));// is
			}
		
			//alert("readBinaryData "+bytes+" ,  value "+value);
			return value;
		};
...
SFInputStreamBinary.prototype["readBinaryData"]=function(n,bitSize){
			...
			for (var i = 0; i < n; i++) {
				...
				for (var j = 0; j < byteSize; j++) {
					...
					var byteValue = bytes[i * byteSize + j];
					value += ((byteValue >= 0 ? byteValue : 256 + byteValue) << (8 * j));// is
				}
				data[i] = value;
			}
			...
			//alert("readBinaryData ("+n+","+bitSize+","+byteSize+")"+data);
