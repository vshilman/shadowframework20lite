...
public class ShadowRectangularObstacle implements ShadowObstacle {
	...
	public ShadowRectangularObstacle(float posx,float posy,float posz, float d1x,float d1y,float d1z) {
		super();
		this.position.set3f(posx,posy,posz);
		this.dim.set3f(d1x,d1y,d1z);
	}
	
	private ShadowRectangularObstacle(){
...
//		gl.glEnd();
	...
	public static boolean isParallelepipedInOccluderShadow(SFVertex3f F,ShadowOccluder occ,SFVertex3f p,SFVertex3f dim){
		
		//when is a rect occluded? when the center of the rect is and there are not intersections 

		float rap=30;
		SFVertex3f[] vsOcc={
				new SFVertex3f(occ.p.getX(),
							   occ.p.getY(),
							   occ.p.getZ()),
				new SFVertex3f(occ.p.getX()+occ.D2.getX(),
							   occ.p.getY()+occ.D2.getY(),
							   occ.p.getZ()+occ.D2.getZ()),
				new SFVertex3f(occ.p.getX()+occ.D2.getX()+occ.D1.getX(),
							   occ.p.getY()+occ.D2.getY()+occ.D1.getY(),
							   occ.p.getZ()+occ.D2.getZ()+occ.D1.getZ()),
				new SFVertex3f(occ.p.getX()+occ.D1.getX(),
							   occ.p.getY()+occ.D1.getY(),
							   occ.p.getZ()+occ.D1.getZ()),
				
				new SFVertex3f(F.getX()+(occ.p.getX()-F.getX())*rap,
							   F.getY()+(occ.p.getY()-F.getY())*rap,
							   F.getX()+(occ.p.getZ()-F.getZ())*rap),
				new SFVertex3f(F.getX()+(occ.p.getX()+occ.D2.getX()-F.getX())*rap,
							   F.getY()+(occ.p.getY()+occ.D2.getY()-F.getY())*rap,
							   F.getZ()+(occ.p.getZ()+occ.D2.getZ()-F.getZ())*rap),
				new SFVertex3f(F.getX()+(occ.p.getX()+occ.D2.getX()+occ.D1.getX()-F.getX())*rap,
							   F.getY()+(occ.p.getY()+occ.D2.getY()+occ.D1.getY()-F.getY())*rap,
							   F.getZ()+(occ.p.getZ()+occ.D2.getZ()+occ.D1.getZ()-F.getZ())*rap),
				new SFVertex3f(F.getX()+(occ.p.getX()+occ.D1.getX()-F.getX())*rap,
							   F.getY()+(occ.p.getY()+occ.D1.getY()-F.getY())*rap,
							   F.getZ()+(occ.p.getZ()+occ.D1.getZ()-F.getZ())*rap),
		...
		boolean intersecting=(new ShadowRectangularObstacle()).isOccluded(p.getX(),p.getY(),p.getZ(), recDimx, recDimy, recDimz, 
				vsOcc);
	...
	public static boolean isParallelepipedInCameraBound(SFCamera cam,SFVertex3f p,SFVertex3f dim){
		...
		float recDimy=1.0f/dim.getY();
		float recDimz=1.0f/dim.getZ();
		boolean intersecting=(new ShadowRectangularObstacle()).hasIntersections(p.getX(),p.getY(),p.getZ(), recDimx, recDimy, recDimz, vsCam);
