...
public class SFBinaryVertexArrayList<T extends SFGenericFixedFloat> extends SFPrimitiveType implements
		SFCharsetObjectList {
	...
	public SFValuenf getValue(int index,int vertexIndex,SFValuenf value){
		int pos=index*totalSize;
		for (int i = 0; i < vertexIndex; i++) {
			pos+=vertexSize[i];
		}
		for (int i = 0; i < vertexSize[vertexIndex]; i++) {
			value.get()[i]=dataObject.get(pos+i).getFloat();
		}
		return value;
	}
