...
public abstract class SFValue {
	...
	/**
	 * Euclidean Distance in the space of the value with the least number of components
	 * @param value
	 * @return
	 */
	public float distance(SFValue value){
		float dotDist = 0;
		for (int i = 0; i < getV().length; i++) {
			float diff= getV()[i] - value.getV()[i];
			dotDist += diff*diff;
		}
		return (float)(Math.sqrt(dotDist));
	}

	/**
	 * Dot product between a point and the difference between two other points.
	 * 
	 * @param value first point 
	 * @param value secodn point
	 * @return dot(this,value-ref)
	 */
	public float triangularDot(SFValue value,SFValue ref){
		float dotDist = 0;
		for (int i = 0; i < getV().length && i<ref.getV().length && i<value.getV().length; i++) {
			float diff= value.getV()[i]-ref.getV()[i];
			dotDist += getV()[i]*diff;
		}
		return dotDist;
	}
