...
public class SFTransform3f extends SFValue {
	...
	public void reverseDirectionTransform(SFValue position){
		float x=position.getV()[0];
		float y=position.getV()[1];
		float z=position.getV()[2];
		
		SFMatrix3f matrix3f=new SFMatrix3f();
		getMatrix(matrix3f);
		matrix3f=SFMatrix3f.getInverse(matrix3f);
		position.getV()[0]=x*matrix3f.getV()[0]+y*matrix3f.getV()[1]+z*matrix3f.getV()[2]+getV()[9];
		position.getV()[1]=x*matrix3f.getV()[3]+y*matrix3f.getV()[4]+z*matrix3f.getV()[5]+getV()[10];
		position.getV()[2]=x*matrix3f.getV()[6]+y*matrix3f.getV()[7]+z*matrix3f.getV()[8]+getV()[11];
	}
	public void reverseTransform(SFValue position){
		float x=position.getV()[0]-getV()[9];
		float y=position.getV()[1]-getV()[10];
		float z=position.getV()[2]-getV()[11];
		
		SFMatrix3f matrix3f=new SFMatrix3f(getV()[0],getV()[1],getV()[2],
	...
	public SFTransform3f getInverseTransform(){
		SFMatrix3f matrix=new SFMatrix3f();
		SFVertex3f vertex=new SFVertex3f();
		getMatrix(matrix);
		getPosition(vertex);
		
		matrix=SFMatrix3f.getInverse(matrix);
		vertex=matrix.Mult(vertex);
		vertex.mult3f(-1);
		
		SFTransform3f inverseTransform=new SFTransform3f();
		inverseTransform.setMatrix(matrix);
		inverseTransform.setPosition(vertex);
