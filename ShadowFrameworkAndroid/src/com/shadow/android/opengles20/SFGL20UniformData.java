...
import com.shadow.android.opengles20.images.SFGL20Texture;
...
public class SFGL20UniformData implements SFProgramDataModel {
	...
	private int[] getUniforms(String prefix,SFPipelineStructureInstance instance,ArrayList<Integer> textures){
		...
		for (int i = 0; i < data.length; i++) {
			...
			data[i]=GLES20.glGetUniformLocation(program.getProgram(),name);
	...
	void evaluateUniforms(SFGL20AbstractProgram program) {
		...
		mainUniforms[0]=GLES20.glGetUniformLocation(program.getProgram(),"projection");
		mainUniforms[1]=GLES20.glGetUniformLocation(program.getProgram(),"modelview");
		mainUniforms[2]=GLES20.glGetUniformLocation(program.getProgram(),"vectorsModelview");
	...
	@Override
	public void setTextureData(Module module, int index, SFPipelineTexture texture) {
		int level=index;
		...
		GLES20.glUniform1i(textures[index], level);
	}
...
//		while(index<8){
//			
//			int textureLevel=GLES20.glGetUniformLocation(program.getProgram(),"texture"+index);
//			if(textureLevel>=0)
//				GLES20.glUniform1i(textureLevel, index);
//			
//			index++;
//		}
//	}
	...
	public void setData(int[] uniforms,SFStructureData data){
		...
		if(uniforms!=null){
			for (int i=0; i < uniforms.length; i++) {
				...
				if(v.getSize()==3){
					GLES20.glUniform3f(uniforms[i],v.get()[0],v.get()[1],v.get()[2]);
				}else if(v.getSize()==1){
					GLES20.glUniform1f(uniforms[i],v.get()[0]);
				}
			}
		}
	}
	...
	public void sendVertex(SFValuenf value) {
		...
		switch (value.getSize()) {
			case 1:
				GLES20.glUniform1f(uniform[0],value.get()[0]);
			break;
			case 2:
				GLES20.glUniform2f(uniform[0],value.get()[0],value.get()[1]);
				break;
			case 3:
				GLES20.glUniform3f(uniform[0],value.get()[0],value.get()[1],value.get()[2]);
				break;
			default:
				break;
		}
			...
			switch (type) {
				case SFPipelineRegister.GLOBAL_FLOAT:
					for (int j=0; j < size; j++) {
						...
						GLES20.glUniform1f(uniform[j],pv.get()[0]);
					}
				break;
				case SFPipelineRegister.GLOBAL_FLOAT3:
						for (int j=0; j < size; j++) {
							SFValuenf pv=(data.getValue(idx[j+position]));
							GLES20.glUniform3f(uniform[j],pv.get()[0],pv.get()[1],pv.get()[2]);
						}
					break;
				case SFPipelineRegister.GLOBAL_FLOAT2:
					for (int j=0; j < size; j++) {
						SFValuenf pv=(data.getValue(idx[j+position]));
						GLES20.glUniform2f(uniform[j],pv.get()[0],pv.get()[1]);
					}
				break;
				default:
					break;
...
//		v3.normalize3f();
		...
		GLES20.glUniformMatrix4fv(mainUniforms[1],1,false,modelview,0);
		GLES20.glUniformMatrix4fv(mainUniforms[2],1,false,vModelview,0);
	}

	public void setupProjetion(float projection[]){
		GLES20.glUniformMatrix4fv(mainUniforms[0],1,false,projection,0);
	}
