...
public class Test002 {
	...
	/**
	 * This test recognize with some patterns all the line of code in House.h
	 * @param args
	 */
	public static void main(String[] args) {
		
		List<String> list=FileStringUtility.loadTextFile("cppTestCode/House.h");
		
		List<CodePattern> patterns=CppCodePatterns.getPatterns();
		
		List<CodeMatch> matches=new ArrayList<CodeMatch>();
		
		for (Iterator<CodePattern> iterator = patterns.iterator(); iterator.hasNext();) {
		...
		for (Iterator<CodeMatch> iterator = matches.iterator(); iterator.hasNext();) {
